...
export const InstagramFeed = ({ posts }) => {
    return (
        <div>
            {posts.map((postID) => <InstagramPost postID={postID}/>)}
            <SeeMore author={"@Instagram"} authorLink={"https://www.instagram.com/instagram"}/>
        </div>
    )
}


// TODO: come up with a better to set individual post height. useScript method does not refresh on feed change
const InstagramPost = ({ postID }) => {
    const uncaptioned = `https://www.instagram.com/p/${postID.id}/embed/`
    const captioned = `https://www.instagram.com/p/${postID.id}/embed/captioned/?cr=1&v=13&wp=477`
    return (
        <iframe 
